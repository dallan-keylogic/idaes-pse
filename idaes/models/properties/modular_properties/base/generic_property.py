...
class GenericStateBlockData(StateBlockData):
    ...
    def calculate_scaling_factors(self):
        ...
        if self.is_property_constructed("cp_mol_phase"):
            for p in self.phase_list:
                ...
                sf_cp_mol_phase = iscale.set_and_get_scaling_factor(
        ...
        if self.is_property_constructed("therm_cond_phase_comp"):
            for p in self.phase_list:
                ...
                for j in self.component_list:
                    sf_therm_cond = iscale.set_and_get_scaling_factor(
                        self.therm_cond_phase_comp[p, j],
                        default=sf_default,
                        warning=True,
                    )
                    if self.config.replace_expressions_with_var_constraint_pairs:
    ...
    def components_in_phase(self, phase):
        ...
        true species set if one exists.
    ...
    def _cp_mol_phase(self):
        try:
            ...
            if self.config.replace_expressions_with_var_constraint_pairs:
                ...
                def cp_mol_phase_eqn(b, p):
                    return b.cp_mol_phase[p] == rule_cp_mol_phase(b, p)
            ...
            if self.is_property_constructed("cp_mol_phase_eqn"):
    ...
    def _therm_cond_phase(self):
        ...
        def rule_therm_cond_phase(b, p):
            ...
            # Handle example when no thermal conductivity is defined for phase
            except GenericPropertyPackageError:
                return None
        ...
        try:
            if self.config.replace_expressions_with_var_constraint_pairs:
                ...
                @self.Constraint(self.phase_list)
                def therm_cond_phase_eqn(b, p):
                    out = rule_therm_cond_phase(b, p)
                    if out is not None:
                        return b.therm_cond_phase[p] == out
                    else:
                        return Constraint.Skip
                ...
                @self.Expression(
                    self.phase_list, doc="Thermal conductivity for each phase"
                )
                def therm_cond_phase(b, p):
                    out = rule_therm_cond_phase(b, p)
                    if out is not None:
                        return out
                    else:
            ...
            if self.is_property_constructed("therm_cond_phase_eqn"):
    ...
    def _therm_cond_phase_comp(self):
        ...
        try:
            if self.config.replace_expressions_with_var_constraint_pairs:
                ...
                @self.Constraint(self.phase_list, self.component_list)
                def therm_cond_phase_comp_eqn(b, p, j):
                    out = rule_therm_cond_phase_comp(b, p, j)
                    if out is not None:
                        return b.therm_cond_phase_comp[p, j] == out
                    else:
                        return Constraint.Skip
                ...
                def therm_cond_phase_comp(b, p, j):
                    out = rule_therm_cond_phase_comp(b, p, j)
                    if out is not None:
                        return out
                    else:
            ...
            if self.is_property_constructed("therm_cond_phase_comp_eqn"):
                self.del_component(self.therm_cond_phase_comp_eqn)
            raise
    ...
    def _visc_d_phase(self):
        ...
        try:
            if self.config.replace_expressions_with_var_constraint_pairs:
                ...
                @self.Constraint(self.phase_list)
                def visc_d_phase_eqn(b, p):
                    out = rule_visc_d_phase(b, p)
                    if out is not None:
                        return out
                    else:
                ...
                @self.Expression(
                    self.phase_list, doc="Dynamic viscosity for each phase"
                )
                def visc_d_phase(b, p):
                    out = rule_visc_d_phase(b, p)
                    if out is not None:
                        return out
                    else:
            ...
            if self.is_property_constructed("visc_d_phase_eqn"):
    ...
    def _visc_d_phase_comp(self):
        ...
        try:
            if self.config.replace_expressions_with_var_constraint_pairs:
                ...
                @self.Constraint(self.phase_list, self.component_list)
                def visc_d_phase_comp_eqn(b, p, j):
                    out = rule_visc_d_phase_comp(b, p, j)
                    if out is not None:
                        return out
                    else:
                        return Constraint.Skip
            ...
            if self.is_property_constructed("visc_d_phase_comp_eqn"):
