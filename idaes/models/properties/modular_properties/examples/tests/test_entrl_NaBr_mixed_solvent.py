...
class TestSymmetricNaBr:
    ...
    @pytest.mark.unit   
    def test_solubility(self, model):
        m = model
...
class TestInfiniteDilutionAqueousNaBr:
    @pytest.fixture(scope="class")
    def model(self):
        m = ConcreteModel()
        ...
        config["components"]["Br-"]["parameter_data"] = {"born_radius": (2.087, pyunits.angstrom)}
    ...
    def test_solubility(self, model):
        ...
        for x, y in data.items():
            n_salt = y / 102.9e-3  # MW of NaBr
            n_H2O = (100 - y) * (x / 100) / 18e-3
            n_EtOH = (100 - y) * ((100 - x) / 100) / 46e-3
            n_total = n_H2O + n_EtOH + 2 * n_salt
...
if __name__ == "__main__":
    ...
    log_Ksp_vec = np.array(log_Ksp_vec)

    plt.plot(x_vec, log_Ksp_vec)
    plt.xlabel("Weight Percent Water")
    plt.ylabel("log K_sp")
    plt.title("Inf. Dil. Water")
    plt.show()