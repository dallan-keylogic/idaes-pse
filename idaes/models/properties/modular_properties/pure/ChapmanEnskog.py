...
class ChapmanEnskogLennardJones(object):
    ...
    class viscosity_dynamic_vap_comp(object):
        ...
        def build_parameters(cobj):
            ...
            if not hasattr(cobj, "collision_integral_callback"):
                cobj.collision_integral_callback = collision_integral_kim_ross_callback
        ...
        def return_expression(b, cobj, T):
            ...
            Omega = cobj.collision_integral_callback(T, cobj)
...
def collision_integral_kim_ross_callback(T, cobj):
    # Properties of Gases and Liquids, Eq. 9.4.5
    units = cobj.parent_block().get_metadata().derived_units
    T = pyunits.convert(T, units["temperature"])
    T_red = T / cobj.lennard_jones_epsilon_reduced
    return 1.604 / pyo.sqrt(T_red)

def collision_integral_neufeld_callback(T, cobj):
    ...
    E = 2.16178
    F = 2.43787
    return A * T_red ** -B + C * pyo.exp(-D * T_red) + E * pyo.exp(-F * T_red)
